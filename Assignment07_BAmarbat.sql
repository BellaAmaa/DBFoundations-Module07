...
-- 2017-01-01,BAmarbat,Created File
--**************************************************************************--
Begin Try
	Use Master;
	If Exists(Select Name From SysDatabases Where Name = 'Assignment07DB_BAmarbat')
	 Begin 
	  Alter Database [Assignment07DB_BAmarbat] set Single_user With Rollback Immediate;
	  Drop Database Assignment07DB_BAmarbat;
	 End
	Create Database Assignment07DB_BAmarbat;
End Try
Begin Catch
	Print Error_Number();
End Catch
go
Use Assignment07DB_BAmarbat;
...
FROM 
    vProducts AS P
JOIN 
    vCategories AS C ON P.CategoryID = C.CategoryID
ORDER BY 
...
SELECT 
    P.ProductName,
    InventoryMonth = FORMAT(I.InventoryDate, 'MMMM, yyyy'),
    I.[Count]
FROM 
    vInventories AS I
JOIN 
    vProducts AS P ON I.ProductID = P.ProductID
ORDER BY 
...
SELECT TOP 1000000
    P.ProductName,
    [InventoryDate] = DATENAME(MM, I.InventoryDate) + ' , ' + DATENAME(YY, I.InventoryDate), [InventoryCount] = I."Count"
...
FROM 
    vCategories AS C
LEFT JOIN vProducts AS P ON C.CategoryID = P.CategoryID
LEFT JOIN vInventories AS I ON P.ProductID = I.ProductID
GROUP BY 
...
LEFT JOIN vInventories AS Prev
    ON I.ProductID = Prev.ProductID
    AND DATEADD(MONTH, 1, Prev.InventoryDate) = I.InventoryDate
ORDER BY 
    1, I.InventoryDate;
go
